...
describe('EnhancedTableRow component', () => {
  ...
  it('should call onRowClick when the row is clicked', async () => {
    ...
    expect(mockedCommonProps.onRowClick).toHaveBeenCalledWith(
      mockedCommonProps.item
    )
  })
  it('should render without rowActions', () => {
    render(
      <table>
        <tbody>
          <EnhancedTableRow {...mockedCommonProps} rowActions={null} />
        </tbody>
      </table>
    )
  ...
  it('should render without onRowClick', async () => {
    render(
      <table>
        <tbody>
          <EnhancedTableRow {...mockedCommonProps} onRowClick={null} />
        </tbody>
      </table>
    )
    const row = screen.getByRole('row')
    await userEvent.click(row)
    expect(mockedCommonProps.onRowClick).not.toHaveBeenCalled()
  })
})
